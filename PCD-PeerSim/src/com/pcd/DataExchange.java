...
//    protected HashMap<String, Integer> receivedData;
    ...
    public void firstCycleInit(Node node, int protocolID) {
        ...
        if (DEBUG_LOGIC) {
            ...
            System.out.println("\tOwned: "+ownedData);
            System.out.println("\tWanted: "+wantedData);
        }
        ...
        for (int i = 0; i < linkable.degree(); i += 1) {
            if (linkable.getNeighbor(i).getID() != node.getID()) {
                if (DEBUG_LOGIC) { System.out.println("\t\t"+linkable.getNeighbor(i).getID()+" connected to "+node.getID());}
    ...
    public void initPeerPolicies() {
        ...
        while (policies.size() < numPolicies && i < (numPolicies * 5)) {
            ...
            String chosenPol = instantiatePolicy(chosenPolRaw,numPolicies);
            ...
            if (!pol.mod.equals("P")) {
                ...
                while (!success && j < 100) {
                    String chosenPolRaw2 = PrologInterface.confProtoPermitPolicies.get(rng.nextInt(PrologInterface.confProtoPermitPolicies.size()));
                    String chosenPol2 = instantiatePolicy(chosenPolRaw2,numPolicies*numPolicies);
    
                    DataPolicy polP = new DataPolicy(peerID,chosenPol2,"",false); 
                    polP.mod = "P";
                    if (addPolicy(polP)) { 
                        success = true;
                    } else {
    ...
    public String instantiatePolicy(String chosenPolRaw, int numPolicies) {
        boolean error = false;
                ...
                if (subs[subID] == null) {
                    try {
                        if (sub.contains("\"")) {
                            ...
                            if (policies.size() < ((numPolicies/4)*3) || rng.nextInt(2) == 0) {
...
//  }
    ...
    private void processMessages(Node node, int protocolID) {
        ...
        for (int i = messages.size() - 1; i >= 0; i -= 1) {
            ...
            if (msg.time <= peersim.core.CommonState.getTime()
                ...
                DataExchange n = (DataExchange) getPeerByID(msg.sender).getProtocol(protocolID);
                if (DEBUG_LOGIC) { System.out.println("Processing Message: "+msg);}
                try {
                    switch (msg.type) {
                        case "DATA_REQUEST":
    ...
    private void processMsg_DataRequest(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        ...
        if (DEBUG_LOGIC) { System.out.println("\tTrans Free: "+transactionFree()+", Has Open In Trans: "+hasOpenInTrans(n.peerID, (String) msg.body[0]));}
        if (transactionFree() && !hasOpenInTrans(n.peerID, (String) msg.body[0])) {       
            if (DEBUG_LOGIC) { 
                System.out.println("\tEntails: "+entails((String) msg.body[0])+" ?= "+msg.body[0]);
                System.out.println("\tOwned: "+ownedData);
                System.out.println("\tWanted: "+wantedData);
            }
            if (entails((String) msg.body[0])) {
                ...
                relPolSets = generatePolicySets(msgSender, (String) msg.body[0], protocolID);
                if (DEBUG_LOGIC) { 
                    System.out.println("\tPolicy Sets: "+relPolSets.size());
                    for (PolicySet ps : relPolSets) {
                        System.out.println(ps.toString());
                    }
                }
                if (relPolSets.size() > 0 || PrologInterface.confDefaultPermit) {
                    if (DEBUG_LOGIC) { System.out.println("\tPOLICY_INFORM");}
                    ...
                    n.sendMessage(protocolID, msgSender, node, newTID, msg.reqTransId, "POLICY_INFORM", new Object[] { (String) msg.body[0], relPolSets }, null);
                } else {
                    if (DEBUG_LOGIC) { System.out.println("\tNO_ACCESS");}
                ...
                DataPackage datalessPackage = assembleDataPackage(null,generateTransactionRecords(),msg.sender);
                n.sendMessage(protocolID, msgSender, node, -1, msg.reqTransId, "NO_DATA", new Object[] { datalessPackage }, null);         
            }
    ...
    private void processMsg_NoData(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        ...
        processIncomingDataPackage((DataPackage) msg.body[0], getPeerByID(msg.sender), protocolID);
    ...
    private Double policyProfitReq(DataPolicy pol) {
        ...
        switch (pol.mod) {
            ...
            case "F":
                break;
            case "O":
                double violObl = pol.penalty * breakMult;
                double fulfilObl = 0;
...
//    %     Represent a state contained as a condition of any of the policies in the set.
    ...
    private void processMsg_RecordInform(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        ...
        if (!hasOpenInTrans(n.peerID, msg.prvTransId)) {
            n.sendMessage(protocolID, msgSender, node, msg.prvTransId, msg.reqTransId, "INVALID_TRANSACTION", new Object[] { }, null);            
        } else {
            ...
            if (relRecords == null) {
                n.sendMessage(protocolID, msgSender, node, msg.prvTransId, msg.reqTransId, "MALFORMED_RECORDS", new Object[] { }, null);
    ...
    private void processMsg_DataResult(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        ...
        processIncomingDataPackage(dataPackage,getPeerByID(msg.sender),protocolID);
    ...
    private void processMsg_Wait(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        if (hasOpenInRemoteTrans(n.peerID, msg.reqTransId)) {
            n.sendMessage(protocolID, getPeerByID(msg.sender), node, msg.prvTransId, msg.reqTransId, "CONFIRM_WAIT", new Object[] { }, null);
        }
    }
    ...
    private void processMsg_MalformedRecords(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        ...
        if (hasOpenOutTrans(n.peerID, msg.reqTransId)) {
            ...
            n.sendMessage(protocolID, node, getPeerByID(msg.sender), msg.prvTransId, msg.reqTransId, "POLICY_INFORM", new Object[] { t.predicate, t.policySets }, null);
        }
    }
    ...
    private void processMsg_PeerDown(DataExchange n, P2PMessage msg, Node node, int protocolID) {
        ...
        if (overlayNetwork.containsKey("peer" + msg.sender)) {
            overlayNetwork.remove("peer" + msg.sender);
        ...
        if (posProfit) {
            ...
            for (ActionSet aSet : todo) {
                ...
                if (chosen == null || aSet.dln < minDln) {
                    if ((aSet.dln - aSet.duration) > CommonState.getTime() && (aSet.duration * PrologInterface.confCycleCost) < (aSet.pen * breakMult)) {
                        minDln = aSet.dln;
                        chosen = aSet;                        
                    }
                }
...
//        }
        ...
        if (faulty && (PrologInterface.confFaultRate == 100 || rng.nextInt((100-PrologInterface.confFaultRate)) == 0)) {
            ...
            if (r.isUp() && !disconnecting) {
                ...
                if (!sDE.disconnecting) {
                    ...
                    sDE.messages.add(new P2PMessage(r.getID(), s.getID(), pTrId, rTrId, "PEER_DOWN", (peersim.core.CommonState.getTime() + 1), new String[] { returnData }));
    ...
    protected void gracefulDisconnect(Node node, int protocolID, int dType) {
        ...
        for (P2PMessage msg : messages) {
            Node msgSender = getPeerByID(msg.sender);
            DataExchange n = (DataExchange) msgSender.getProtocol(protocolID);
            n.sendMessage(protocolID, msgSender, node, msg.prvTransId, msg.reqTransId, "PEER_DOWN", new String[] { (String) msg.body[0] }, null);
        }
...
//        }
    ...
    private Node getPeerByID(long peerID) {
        return getPeerByID("peer"+peerID);
    }
