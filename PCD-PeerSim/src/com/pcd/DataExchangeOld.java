...
public class DataExchangeOld implements CDProtocol {
    ...
    public DataExchangeOld(String prefix) {
    ...
    /*
    Here we decide whether policies will be fulfilled, or broken. But this decision is never used
     */
    /*public float[] policyCostRequestor(P2PMessage msg, HashSet<Term> relPolicies) {        
        ...
        if (DEBUG_POL_COST) { System.out.println("\n==========POLICY COST EVALUATION==========");}
        if (DEBUG_POL_COST) { System.out.println("peer"+peerID+" is requesting "+msg.payload[1]+" x "+msg.payload[0]+" from peer"+msg.sender.getID()+" (Cycle "+peersim.core.CommonState.getTime()+")");}
        ...
        for (Term pol : applicablePolicies) {
            ...
            //if (DEBUG_POL_COST) { System.out.println(tempPol.toString());}
        ...
        int sdValue = ((cycleCost * AVG_TRANS_LENGTH) * ((numNeighboursWithoutData / overlayNetwork.size()) * 10));
        
        //requestRecords(I,ID,D,L,M,N)
        
        
        if (DEBUG_POL_COST) { System.out.println("Data Value (- Transaction Cost): ("+lDataValue+" * "+maxRecordsLowest+") = "+(lDataValue*maxRecordsLowest)+" - "+(cycleCost*AVG_TRANS_LENGTH-1));}
        if (DEBUG_POL_COST) { System.out.println("Supply and Demand Value: ( ("+cycleCost+" * "+AVG_TRANS_LENGTH+") * (("+numNeighboursWithoutData+" / "+overlayNetwork.size()+") * 10) ) = "+sdValue);}
        
        float policyCost = (lDataValue*maxRecordsLowest) + sdValue - (cycleCost*AVG_TRANS_LENGTH-1);
        ...
        if (DEBUG_POL_COST && polOblSets.size() > 0) { System.out.println("Fulfill Calc:");}
        for (ObligationSet oblSet : polOblSets) {
            float[] oblCosts = obligationCost(oblSet);
            if (oblSet.preObligation) { 
                policyCost -= oblCosts[0];
            } else {
                fulfilOblCost += oblCosts[0];
            }
            breakOblCost += oblCosts[1];
        }
        ...
        if (DEBUG_POL_COST) { System.out.println("Policy Value: "+policyCost);}
        if (DEBUG_POL_COST) { System.out.println("==========================================\n");}
        
        return new float[]{policyCost, fulfilOblCost, breakOblCost};
    }*/
    
    /*public float[] policyCostProvider(String dataItem, int dataQuantity, HashSet<Term> relPolicies) {
        float fulfilPermitCost = 0, fulfilProhibitCost = 0, breakPermitCost = 0, breakProhibitCost = 0, obligationReward = 0;
        for (Term pol : relPolicies) {
            DataPolicy tempPol = new DataPolicy(peerID,pol,0,false);
            switch (tempPol.safeModality) {
                case "P":
                    fulfilPermitCost -= tempPol.reward;
                    fulfilPermitCost += (cycleCost * AVG_TRANS_LENGTH);
                    breakPermitCost += tempPol.penalty;
                    break;
                case "F":
                    fulfilProhibitCost -= tempPol.reward;
                    breakProhibitCost += tempPol.penalty;
                    break;
                case "O":*/
                    /*
                    %If Obl is Obtain  // obtain(Data, Quantity)
                        //No Computable Benefit
    
                    %If Obl is Provide and Obl.Peer targets you // provide(Data, Quantity, Peer)   
                        %ObligationReward += Value of Obl.Data * Obl.Quantity
    
                    %If Obl is Adopt // adopt(Policy, Duration)
                        %// Deeper analysis could go here, looking at:
                            %Data this new policy will allow you to access
                            %Extra data trade from people who cannot get data from the requestor due to this new policy, which you can provide
                            %The pay off of obligations attached to this policy (limit recursion on this one)
    
                    %If Obl is Inform // inform(Peer)
                        %//No Computable Benefit
    
                    %// When passing on an obligation imposed on you by another, do you remove that obligation from yourself?
                    %// Yes, you can pass on an obligation, but you also pass on any reward/penalty in the process
                    %If this is an obligation enforced on you by another that you are passing on
                        %CompletionCost <- CompletionCost(Obl) //Returns [CyclesToComplete,CostToComplete,ProbabilityToComplete]
                        %ObligationReward += CompletionCost[1]
                        %ObligationReward += Obl.Penalty * (1 - CompletionCost[2]) //Chance of failing to complete obligation in time
                    *//*
                    break;
            }
        }
        
        float permitCost = fulfilPermitCost + breakProhibitCost - obligationReward;
        float forbidCost = breakPermitCost + fulfilProhibitCost;  
        
        return new float[]{permitCost, forbidCost};
    }*/
    
    /*public float[] obligationCost(ObligationSet oblSet) {
        if (DEBUG_POL_COST) { System.out.println("\tObligation Set [Pen: "+oblSet.penalty+", Dur: "+oblSet.duration+", Pre: "+oblSet.preObligation+"]");}
        //Estimated Cost to fulfill obligations
        //This should also take into account current obligations, and how they will affect/be affected
        ...
        for (Obligation obl : oblSet.obligations.keySet()) {      
            if (DEBUG_POL_COST) { System.out.println("\t\t"+obl);}
            ...
            switch (obl.type) {
                ...
                // obtain(d3, 4)    
                    ...
                    int dataVal = getDataValue(obl.payload[0]);
                    
                    if (obl.type.equals("obtain") || (obl.type.equals("provide") && dataCount < dataQuantity)) {                                            
                        //Cost
                        if (dataCount < dataQuantity) { 
                            fulfilThisOblCost += ((cycleCost * AVG_TRANS_LENGTH));
                            if (DEBUG_POL_COST) { System.out.println("\t\t\tObtain Cost (Cycles): "+(cycleCost * AVG_TRANS_LENGTH));}
                        }
                        fulfilThisOblCost -= (dataVal * dataQuantity);
                        minCyclesNeeded += AVG_TRANS_LENGTH;
                        
                        if (DEBUG_POL_COST) { System.out.println("\t\t\tObtain Cost (Value): ("+dataVal+" * "+dataQuantity+") = "+(dataVal * dataQuantity));}
                                ...
                                if (oblTest.type.equals("adopt")) {
                                    ...
                                    if (tempPolicy.selfTarget) {
                                        ...
                                        if (!selfPolPermit) {
                                            ...
                                            if (DEBUG_POL_COST) { System.out.println("\t\t\t\t(Obtain) Cost to Break a Self Policy: "+oblSetTest.penalty);}
                                            break;
                                        }
                                    }
                                }
                    ...
                    if (obl.type.equals("obtain")) {                             
                        if (DEBUG_POL_COST) { System.out.println("\t\t\tCycles: "+AVG_TRANS_LENGTH);}
                        if (DEBUG_POL_COST) { System.out.println("\t\t\tProbability: "+completionProb);}
                        break;
                    }
                ...
                // provide(d1, 5, peer80)
                    //Cost
                    fulfilThisOblCost += (cycleCost * AVG_TRANS_LENGTH);
                    fulfilThisOblCost += ((dataVal/10) * dataQuantity);
                    minCyclesNeeded += AVG_TRANS_LENGTH;

                    if (DEBUG_POL_COST) { System.out.println("\t\t\tProvide Cost (Cycles): "+(cycleCost * AVG_TRANS_LENGTH));}
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tProvide Cost (Value): ("+(dataVal/10)+" * "+dataQuantity+") = "+((dataVal/10) * dataQuantity));}
                        ...
                        for (Obligation oblTest : oblSetTest.obligations.keySet()) {
                            if (oblTest.type.equals("adopt")) {
                                ...
                                if (!tempPolicy.selfTarget) {
                                    ...
                                    if (!polPermit) {
                                        fulfilThisOblCost += oblSetTest.penalty;
                                        if (DEBUG_POL_COST) { System.out.println("\t\t\t\tCost to Break a Policy: "+oblSetTest.penalty);}
                                        break;
                                    }
                                }
                            }
                ...
                // adopt(['P',any,d3,[],[],[[[inform(peer85)], 5, 6]],false], 5)
                    ...
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tAdopt Cost (Cycles): "+(cycleCost * Integer.parseInt(obl.payload[1]+1)));}
                    ...
                    for (ObligationSet oblSetTest : obligationSets) {
                        for (Obligation oblTest : oblSetTest.obligations.keySet()) {
                            if (oblTest.type.equals("adopt")) {
                                ...
                                if (polViolated) {
                                    if (DEBUG_POL_COST) { System.out.println("\t\t\t\t(Adopt) Cost of Violated Policy: "+oblSetTest.penalty);}
                                    fulfilThisOblCost += oblSetTest.penalty;
                                    break;
                                }
                            }
                    ...
                    for (String d : desiredData.keySet()) {
                        if (!PrologInterface.runGroundQuery("permitRequestPolicy",new Term[]{new Atom("peer"+peerID),new Atom(d),adoptOblPolicy.getPrologTerm()})) {
                            ...
                            fulfilThisOblCost += Math.min((cycleCost * (AVG_TRANS_LENGTH + Integer.parseInt(obl.payload[1]))),dVal);

                            for (ObligationSet oblSetTest : obligationSets) {
                                for (Obligation oblTest : oblSetTest.obligations.keySet()) {
                                    if ((oblTest.type.equals("obtain") || oblTest.type.equals("provide")) && oblTest.payload[0].equals(d)) {
                                        fulfilThisOblCost += oblSetTest.penalty;
                                        if (DEBUG_POL_COST) { System.out.println("\t\t\t\t(Adopt) Cost of Blocked Obligation: "+oblSetTest.penalty);}
                                        break;
                                    }
                                }
                            }
                    ...
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tCycles: -");}
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tProbability: "+completionProb);}
                    break;
                ...
                // inform(peer0)
                    ...
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tInform Cost (Cycles): "+cycleCost);}
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tCycles: 1");}
                    if (DEBUG_POL_COST) { System.out.println("\t\t\tProbability: "+completionProb);}
                    break;
            }
        }
        ...
        if (!oblSet.preObligation) {
            if (DEBUG_POL_COST) { System.out.print("\t\tDuration: "+minCyclesNeeded+" > "+oblSet.duration);}
            if (minCyclesNeeded > oblSet.duration) { 
                completionProb = 0.0;
                if (DEBUG_POL_COST) { System.out.print(" -> Completion is impossible [Prob: "+completionProb+"]");}
            } else {
                long spareRounds = Math.abs(minCyclesNeeded-oblSet.duration);
                if (spareRounds <= 2) {
                    if (DEBUG_POL_COST) { System.out.print("\n\t\t\tTight Schedule -> Completion = "+completionProb+" * "+(0.5+(spareRounds*0.2)));}
        ...
        fulfilThisOblCost += (cycleCost * minCyclesNeeded);
        if (DEBUG_POL_COST) { System.out.println("\t\tCompletion Cycle Cost: "+minCyclesNeeded+" * "+cycleCost+" = "+(minCyclesNeeded*cycleCost));}
        
        fulfilThisOblCost += ((oblSet.penalty * cycleCost) * (1.0 - completionProb));
        if (DEBUG_POL_COST) { System.out.println("\t\tCompletion Probability: "+completionProb);}
        if (DEBUG_POL_COST) { System.out.println("\t\tCompletion Value: (("+oblSet.penalty+" * "+cycleCost+") * (1.0 - "+completionProb+")) = "+((oblSet.penalty * cycleCost) * (1.0 - completionProb)));}
        if (DEBUG_POL_COST) { System.out.println("\t\tCompletion Cost: "+fulfilThisOblCost);}
                
        float breakThisOblCost = 0;
        if (!oblSet.preObligation) {
            breakThisOblCost += (oblSet.penalty * cycleCost);
        }
        
        return new float[] {fulfilThisOblCost, breakThisOblCost};
    }*/
